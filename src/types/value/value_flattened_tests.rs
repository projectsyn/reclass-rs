...
fn test_flattened_nested_mapping() {
    ...
    let f = m.rendered(&Mapping::new(), &Mapping::new()).unwrap();
...
fn test_flattened_nested_mapping_value_list() {
    ...
    let f = v.rendered(&Mapping::new(), &Mapping::new()).unwrap();
    assert!(f.is_mapping());
    let m: serde_yaml::Mapping = f.as_mapping().unwrap().clone().into();
    let expected =
...
fn test_flattened_nested_mapping_value_list_3() {
    ...
    let f = Value::Mapping(dbg!(base))
        .rendered(&Mapping::new(), &Mapping::new())
        .unwrap();
    assert!(f.is_mapping());
    let m: serde_yaml::Mapping = f.as_mapping().unwrap().clone().into();
    let expected =
...
fn test_flatten_value_list() {
    ...
    v.render(&Mapping::new(), &Mapping::new()).unwrap();
    assert!(v.is_mapping());
